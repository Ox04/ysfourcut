...
> **2026-09-07 디자인 시스템 v2 적용(사용자 승인, Fable 세션).** 시각 방향이
> "영수증 티켓(종이색·둥근 모서리·그림자)"에서 **쿨 그레이 v2(stone 중립 + 셔터 버밀리언,
> 모서리 0, 경계선 구획, Wanted Sans)**로 교체됐다. 아래 07/12 기록의 화면 구조·컴포넌트·
> 접근성·검증 내용은 유효하지만 **색·모서리·그림자·폰트 값은
> `docs/DESIGN_SYSTEM_PROPOSAL.md`와 `src/ds/ds.css`(원본)·`styles/tokens.css`(사본)가
> 기준이다.** 절취선 모티프와 서버 PNG 원본 표시 원칙은 유지.
> 컴포넌트 데모: 개발 실행에서 `#design`.
